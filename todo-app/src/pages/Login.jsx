...
const Login = ({ setIsAuth }) => {
  ...
  const handleFamilyCodeAccess = async () => {
    if (!familyCode.trim()) {
      toast.error("Veuillez entrer un code familial")
      return
    }
    ...
    try {
      ...
      if (!familyDoc.exists()) {
        toast.error("Code familial invalide")
        setLoading(false)
        return
      }
      ...
      if (hasMembers) {
        ...
        toast.success("Connexion réussie ! Sélectionnez votre profil.")
        setLoading(false)
        return
      }

      toast.error("Aucun membre trouvé dans cette famille")
      setLoading(false)
    } catch (err) {
      toast.error("Erreur lors de l'accès à la famille : " + err.message)
      setLoading(false)
    }
  }
  ...
  const handleSuccessfulAuth = async (user) => {
    ...
    if (showFamilyCodeInput && familyCode.trim()) {
      ...
      // Connexion normale - vérifier le profil utilisateur
      ...
      if (!userDocSnap.exists() || !userDocSnap.data().completedProfile) {
        ...
        // Marquer le type de connexion comme normal et nettoyer les données d'invité
        ...
        // Mettre à jour le type de connexion dans la base de données
        await setDoc(
          userDocRef,
          {
            connectionType: "normal",
            isGuest: false,
            isReadOnly: false, // Utilisateur normal, pas en lecture seule
          },
          { merge: true },
        )
  ...
  const handleProfileSelect = async (member) => {
    ...
    try {
      ...
      if (isGuestMode) {
        // Mode invité - accès en lecture seule avec code famille
        try {
          ...
          // Attendre un court instant pour montrer l'animation
        ...
        // Mode normal avec authentification - connexion avec code famille mais utilisateur authentifié
        const userDocRef = doc(db, "users", currentUser.uid)
        await setDoc(
          userDocRef,
          {
        ...
        // Stocker le type de connexion et l'état d'authentification
          ...
          <div className="px-8 py-6">
            ...
            <div className="space-y-4">
              <label className="flex items-center space-x-2 text-sm font-medium text-gray-700 cursor-pointer">
                ...
                />
                <span>Accéder avec un code famille</span>
              </label>
              ...
              {showFamilyCodeInput ? (
                ...
                <form onSubmit={signInWithEmailAndPasswordHandler} className="space-y-4">
                  <div className="relative">
